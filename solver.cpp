...
void network::solvePressures()
{
            ...
            for(unsigned j=0;j<neighboors.size();++j)
            {
                ...
                if(!p->getClosed())
                {
                    if(p->getInlet())
                    {
                        ...
                        //cout<<b(row)<<" = b in"<<endl;
                        conductivity-=p->getConductivity();
                        //cout<<"conductivity in = "<<conductivity<<endl;
                    }
                    if(p->getOutlet())
                    {
                        b(row)+=-pressureOut*p->getConductivity();
                        //cout<<b(row)<<" = b out artery"<<endl;
                        conductivity-=p->getConductivity();
                        //cout<<"conductivity out = "<<conductivity<<endl;
                    }
                    if(!p->getInlet() && !p->getOutlet())
                    {
                        ...
                        if(!neighboor->getClosed())
                        {
                            ...
                            conductivity-=p->getConductivity();
                            //cout<<"conductivity other = "<<conductivity<<endl;
                        }
                    }
                }
            }
    ...
    if(solverChoice==1)
    {
        SimplicialLDLT<SparseMatrix<double> > solver;
        solver.compute(conductivityMatrix);
        pressures=solver.solve(b); //cout<<b<<endl;
        //cout<<"pressures = "<<pressures<<endl;
...
{
    double outletFlow(0);
    //cout<<"totalPores = "<<totalPores<<endl;
    for(int i=0;i<totalPores;++i)
    {
        ...
        if(!p->getClosed())
        {
            if(p->getOutlet())
            {
                ...
                p->setAveragePressure((p->getNodeOut()->getPressure()+pressureOut)/2);
            ...
            if(!p->getInlet() && !p->getOutlet())
            {
                ...
                p->setAveragePressure((p->getNodeIn()->getPressure()+p->getNodeOut()->getPressure())/2);
            }
        }
    }
    return outletFlow;
