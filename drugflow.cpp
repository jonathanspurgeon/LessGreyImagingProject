...
void network::runDrugFlowWithoutDiffusion()
{
    ...
    cout<<"Starting Flow in Mouse Network... "<<endl;
    ...
    for(int i=0;i<totalNodes;++i)
    {
        ...
        if(!n->getClosed())
        {
            ...
            if(abs(sumFlow)>1e-20)
            {
                ...
                cout<<sumFlow<<endl;
                cout<<n->getId()<<" "<<n->getVesselType() <<" "<<n->getOutlet()<<" "<<n->getInlet()<<endl;
                cancel=true;
//                }
    ...
    while(timeSoFar<simulationTime)
    {
        ...
        for(int i=0;i<totalNodes;++i)
        {
            ...
            if(!n->getClosed())
            {
                ...
                for(unsigned j=0;j<connectedPores.size();++j)
                {
                    ...
                    if(!p->getClosed())
                    {
                        if((p->getNodeIn()==n && p->getFlow()>0) || (p->getNodeOut()==n && p->getFlow()<0))
                        {
        ...
        for(int i=0;i<totalPores;++i)
        {
            ...
            if(!p->getClosed())
            {
                if(p->getInlet()) // inlet pore
                {
                    ...
                    double concentration=bolusInjection? 1:A1/(sigma1*sqrt(2*tools::pi()))*exp(-pow((time-T1),2)/(2*pow(sigma1,2)))+A2/(sigma2*sqrt(2*tools::pi()))*exp(-pow((time-T2),2)/(2*pow(sigma2,2)))+alpha*exp(-beta*time)/(1+exp(-s*(time-tau)));
                ...
                p->setConcentration(p->getConcentration()+(abs(p->getFlow())/flowIn*massIn-abs(p->getFlow())*p->getConcentration())*timeStep/p->getVolume());
        ...
        for(int i=0;i<totalNodes;++i)
        {
            ...
            if(!n->getClosed())
            {
               ...
               // if(n->getConcentration()<-0.001 || n->getConcentration()>1.001) cout<<"nfuck"<<n->getConcentration()<<endl;
            }
        }
        ...
        if(extractData && outputPV2>0.4*totalPoresVolume)
        {
            ...
            extractDrugFlowResults(endTime-startTime, timeSoFar, simulationTimeElapsed, outputCount,true);
        }

        //std::this_thread::sleep_for(std::chrono::milliseconds(5));

        if(bolusInjection && timeSoFar>bolusDuration)
            removeTracer=true;
...
void network::runDrugFlowWithDiffusion()
{
    ...
    {
        block* n=getBlock(i);
        file4<<n->getId()<<" "<<n->getXCoordinate()<<" "<<n->getYCoordinate()<<" "<<n->getZCoordinate()<<" "<<n->getDiffusivity()<<endl;
        file5<<n->getId()<<" "<<n->getX()<<" "<<n->getY()<<" "<<n->getZ()<<endl;
    }
        ...
        if(!p->getClosed())
        {
            if(!p->getInlet()) // inlet pore
            {
                ...
                for(unsigned j=0;j<connectedPores.size();j++)
                {
                    ...
                    if(!npore->getClosed())
                    {
                        if((p->getFlow()>0 && npore->getFlow()>0 && npore->getNodeIn()==p->getNodeOut()) ||
                           (p->getFlow()>0 && npore->getFlow()<0 && npore->getNodeOut()==p->getNodeOut()) ||
                           (p->getFlow()<0 && npore->getFlow()>0 && npore->getNodeIn()==p->getNodeIn()) ||
                           (p->getFlow()<0 && npore->getFlow()<0 && npore->getNodeOut()==p->getNodeIn())
                                )
                        {
    ...
    while(timeSoFar<simulationTime)
    {
            ...
            if(!n->getClosed())
            {
                ...
                for(auto iterator : neigh)
                {
                    ...
                    if(!pp->getClosed())
                    {
                        double area=iterator.second;
                        sumSource+=pp->getMembranePermeability()*area/(n->getVolume());
                        sumSource2+=pp->getConcentration()*pp->getMembranePermeability()*area/(n->getVolume());
                    }
                }

                double newConcentration(0);

                newConcentration+=n->getConcentration()*(1-deltaT*(2*n->getDiffusivity()*coeff+sumSource+sigma));
                if(nW!=0) if(!nW->getClosed()) newConcentration+=deltaT*n->getDiffusivity()*coefX*nW->getConcentration();
                if(nE!=0) if(!nE->getClosed()) newConcentration+=deltaT*n->getDiffusivity()*coefX*nE->getConcentration();
                if(nS!=0) if(!nS->getClosed()) newConcentration+=deltaT*n->getDiffusivity()*coefY*nS->getConcentration();
                if(nN!=0) if(!nN->getClosed()) newConcentration+=deltaT*n->getDiffusivity()*coefY*nN->getConcentration();
                if(nD!=0) if(!nD->getClosed()) newConcentration+=deltaT*n->getDiffusivity()*coefZ*nD->getConcentration();
                if(nU!=0) if(!nU->getClosed()) newConcentration+=deltaT*n->getDiffusivity()*coefZ*nU->getConcentration();

                if(closedBoundaries)
                {
                    if(nW==0) newConcentration+=deltaT*n->getDiffusivity()*coefX*n->getConcentration();
                    if(nE==0) newConcentration+=deltaT*n->getDiffusivity()*coefX*n->getConcentration();
                    if(nS==0) newConcentration+=deltaT*n->getDiffusivity()*coefY*n->getConcentration();
                    if(nN==0) newConcentration+=deltaT*n->getDiffusivity()*coefY*n->getConcentration();
                    if(nD==0) newConcentration+=deltaT*n->getDiffusivity()*coefZ*n->getConcentration();
                    if(nU==0) newConcentration+=deltaT*n->getDiffusivity()*coefZ*n->getConcentration();
                }
            ...
            if(!p->getClosed())
            {
                ...
                for(auto iterator : neigh)
                {
                    ...
                    if(!bb->getClosed())
                    {
                        double area=iterator.second;
                        sumSource+=p->getMembranePermeability()*area/(p->getVolume());
                        sumSource2+=bb->getConcentration()*p->getMembranePermeability()*area/(p->getVolume());
                    }
                }
                ...
                else
                {
                    ...
                    for(auto iterator : neighPores)
                    {
                        ...
                        if(!pp->getClosed())
                        {
                            sumInflow+=pp->getConcentration()*p->getInflowShare()*inflow/p->getVolume();
                        }
                    }
                }

                double newConcentration(0);
                newConcentration+=p->getConcentration()*(1-deltaT*(abs(p->getFlow())/p->getVolume()+sumSource));
        ...
        for(int i=0;i<totalBlocks;++i)
        {
            ...
            if(!n->getClosed())
            {
                n->setConcentration(blockConcentration[j]);j++;
                if(n->getConcentration()<0 || n->getConcentration()>1) {cout<<"block concentration out of range: "<<n->getConcentration()<<endl;cancel=true;}
            }
        }
        ...
        for(int i=0;i<totalPores;++i)
        {
            ...
            if(!p->getClosed())
            {
                p->setConcentration(poreConcentration[j]);j++;
                if(p->getConcentration()<0 || p->getConcentration()>1.001) {cout<<"pore concentration out of range: "<<p->getConcentration()<<endl;cancel=true;}
            }
        }
        ...
        double averageConc(0),outletConc(0),averageVoxelConc(0),totalVolume(0),totalOutletVolume(0);
        for(int i=0;i<totalPores;++i)
        {
            pore* p=getPore(i);
            if(!p->getClosed())
        ...
        outputPV2+=timeStep*flowRate;
        if(outputPV>0.04*totalPoresVolume)
        //if((averageConc!=0 && abs(oldAverageConc-averageConc)/averageConc>0.1) || (averageTissueConc!=0 && abs(oldTissueConc-averageTissueConc)/averageTissueConc>0.1))
        {
            outputPV=0;
            ofs1<<timeSoFar<<" "<<outletConc<<endl;
            ofs2<<timeSoFar<<" "<<averageConc<<endl;
