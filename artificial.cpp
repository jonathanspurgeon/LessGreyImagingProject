...
void network::buildArtificialNetwork()
{
    ...
    double xcoordinates[] = {200,170,120,80,50,25,10,10,25,50,80,120,170,200};
    ...
    for(int i=0;i<7;++i)
    {
        ...
        {
            double y=-(abscisse/L)*yEdgeLength/2.+j*(abscisse/L*yEdgeLength)/(index-1);
    ...
    for(int i=7;i<13;++i)
    {
        ...
        for(int j=0;j<index;++j)
        {
            double y=-(yEdgeLength-(abscisse/(2*L))*yEdgeLength)+2*(yEdgeLength-(abscisse/(2*L))*yEdgeLength)*j/(index-1);
            tableOfAllNodes[nodeNumber]= new node(rabscisse,y,0);
            //cout<<nodeNumber<<" "<<abscisse<<" "<<y<<endl;
            nodeNumber++;
        }
    ...
    for(int i=0;i<7;++i)
    {
        for(int j=0;j<index;++j)
        {
            ...
            tableOfAllPores[poreNumber]=new pore(nodeIn,nodeOut1);
           // cout<<poreNumber<<" "<<totalNodes-1-nodeNumber<<" "<<totalNodes-1-2*nodeNumber<<endl;
            poreNumber++;
            tableOfAllPores[poreNumber]=new pore(nodeIn,nodeOut2);
          //  cout<<poreNumber<<" "<<totalNodes-1-nodeNumber<<" "<<totalNodes-1-2*nodeNumber-1<<endl;
            poreNumber++;
            nodeNumber++;
        }
        index*=2;
    ...
    tableOfAllPores[totalPores-1]=new pore(getNode(totalNodes-1),getNode(totalNodes-2));
    ...
    for(int i=0;i<totalNodes;++i)
    {
        ...
        if(connectionNumber>maxConnectionNumber)maxConnectionNumber=connectionNumber;
    }
    ...
    {
        node* n=getNode(i);
        if(n->getId()==1 || n->getId()==2 || n->getId()==totalNodes ||n->getId()==totalNodes-1)continue;
        if(n->getId()<8 ||  n->getId()>=totalNodes-8)
        {
            n->setXCoordinate(n->getXCoordinate()+5*length*degreeOfDistortion*(-1+2*uniform_real()));
            n->setYCoordinate(n->getYCoordinate()+5*length*degreeOfDistortion*(-1+2*uniform_real()));
        }
        else
        {
            n->setXCoordinate(n->getXCoordinate()+length/5*degreeOfDistortion*(-1+2*uniform_real()));
            n->setYCoordinate(n->getYCoordinate()+length/5*degreeOfDistortion*(-1+2*uniform_real()));
        }
    }
    ...
    for (int i = 0; i < totalPores; ++i)
    {
        ...
        if(!p->getClosed())
        {
            double len=sqrt(pow(p->getNodeIn()->getXCoordinate()-p->getNodeOut()->getXCoordinate(),2)+pow(p->getNodeIn()->getYCoordinate()-p->getNodeOut()->getYCoordinate(),2)+pow(p->getNodeIn()->getZCoordinate()-p->getNodeOut()->getZCoordinate(),2));
            p->setLength(len);
            p->setFullLength(len);
            length+=len;
        }
    ...
    for (int i = 0; i < totalPores; ++i)
    {
        ...
        if(!p->getClosed())
        {
            double volume=poreVolumeConstant*p->getLength()*pow(p->getRadius(),poreVolumeExponent)/(4*p->getShapeFactor())*pow(10,(6*poreVolumeExponent-12));
            p->setVolume(volume);
    ...
    for (int i = 0; i < totalPores; ++i)
    {
        ...
        if(!p->getClosed())
        {
            double conductivity=poreConductivityConstant*p->getShapeFactorConstant()*pow(p->getRadius(),poreConductivityExponent)/(16*p->getShapeFactor())/(p->getViscosity()*p->getLength())*pow(10,(6*poreConductivityExponent-24));;
            p->setConductivity(conductivity);
        }
    }

    int closedPoresNumber=int(totalOpenedPores*(1-coordinationNumber/4.0));
    int i=0;
...
void network::buildArtificialNetwork2()
{
    ...
    // Creating Nodes
    totalOpenedNodes=totalNodes=(5+1)*2+(10+1)*4+(50+1)*20+(200+1)*80+(400+1)*160;
    tableOfAllNodes.resize(totalNodes);
    ...
    totalOpenedPores=totalPores=5+1+(5+1)*2+(10+1)*4+(50+1)*20+(200+1)*80+(400+1)*160+(5)*2+(10)*4+(50)*20+(200)*80+(400)*160;
    tableOfAllPores.resize(totalPores);
    double poreNumber(0);
    double nodesSoFar(0);
    nodeNumber=0;
    ...
    int closedPoresNumber=max(0.0,totalOpenedPores*(1-coordinationNumber/4.0));
    int i=0;
    while(i<closedPoresNumber)
    {
        int index=uniform_int(0,totalPores-1);
...
void network::buildArtificialNetwork3() //branches in 3^n + rectangular part
{
   ...
   for(int i=0;i<4;++i)
   {
       ...
       for(int j=0;j<index;++j)
       {
           double y=-(abscisse/L)*yEdgeLength/2.+j*(abscisse/L*yEdgeLength)/(index-1);
           tableOfAllNodes[nodeNumber]= new node(abscisse,y,0);
           //cout<< "nodeNumber:"<<nodeNumber<<" "<<"abscisse:"<< abscisse<<" "<<"y:"<< y<<endl;
           nodeNumber++;
       }
   }
   ...
   cout<<"nodeNumber:"<< nodeNumber<<" "<<"totalNodes:"<< totalNodes<<endl;
   ...
   for(int i=0;i<4;++i)
   {
       for(int j=0;j<index;++j)
       {
           ...
           tableOfAllPores[poreNumber]=new pore(nodeIn,nodeOut1);
          //cout<<"poreNumber:"<< poreNumber<<" "<<"nodeNumber:"<< nodeNumber<<" "<<"2*noedNumber:"<< 2*nodeNumber<<endl;
           poreNumber++;
           tableOfAllPores[poreNumber]=new pore(nodeIn,nodeOut2);
          //cout<<"poreNumber:"<< poreNumber<<" "<<"nodeNumber:"<< nodeNumber<<" "<<"2*noedNumber+1:"<< 2*nodeNumber+1<<endl;
           poreNumber++;
   ...
   for(int i=0;i<25;++i)
   {
       for(int j=0;j<81;++j)
       {
           ...
           tableOfAllPores[poreNumber]=new pore(nodeIn,nodeOut);
           //cout<<"poreNumber:"<< poreNumber<<" "<<"nodeNumber:"<< nodeNumber<<" "<<"nodeNumber+128: "<< nodeNumber+128<<endl;
           poreNumber++;
           nodeNumber++;
       }
   }
           //cout<<"poreNumber after loop:"<< poreNumber<<"nodeNumber after loop:"<< nodeNumber<<endl;
   ...
   for(int i=0;i<25;++i)
   {
       for(int j=0;j<81-1;++j)
       {
           ...
           tableOfAllPores[poreNumber]=new pore(nodeIn,nodeOut);
          //cout<<"poreNumber:"<<poreNumber<<" "<<"nodeNumber:"<< nodeNumber<<" "<<"2*nodeNumber:"<< 2*nodeNumber<<endl;
           poreNumber++;
           //cout<<"nodeNumber:"<< nodeNumber<<" "<<"abscisse:"<< abscisse<<endl; //<<" "<<"y:"<< y<<endl;
           nodeNumber++;
       }
       nodeNumber++;
   }
  ...
  /* index=1;
   ...
   tableOfAllPores[totalPores-1]=new pore(getNode(totalNodes-2),getNode(totalNodes-1));
   ...
   for(int i=0;i<totalNodes;++i)
   {
       ...
       {
           n->setXCoordinate(n->getXCoordinate()+length/5*0.42*(-1+2*uniform_real()));
           n->setYCoordinate(n->getYCoordinate()+length/5*0.42*(-1+2*uniform_real()));
       }
   }
...
void network::buildArtificialNetwork4() //branches in 2^n
{
    ...
    double xcoordinates[] = {200,170,120,80,50,25,10,10,25,50,80,120,170,10};
    ...
    for(int i=0;i<7;++i)
    {
        for(int j=0;j<index;++j)
        {
           ...
           // cout<<poreNumber<<" "<<totalNodes-1-nodeNumber<<" "<<totalNodes-1-2*nodeNumber<<endl;te
            poreNumber++;
...
void network::buildArtificialNetwork6() //small: branches up to 2^2 + rectangular part
{
    maxConnectionNumber=0;
    xEdgeLength=1960.*1e-6;
    yEdgeLength=1280.*1e-6;
...
void network::buildArtificialNetwork7() //small: 2 layers of different densities
{
    maxConnectionNumber=6;
    ...
    totalOpenedPores=totalPores=5+1+(5+1)*2+(10+1)*4+(5)*2+(10)*4;
    tableOfAllPores.resize(totalPores);
    double poreNumber(0);
    double nodesSoFar(0);
    nodeNumber=0;
