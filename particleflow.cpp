...
//                }
    ...
    for(int i=0;i<totalPores;++i)
    {
        ...
        {
            file1<<p->getId()<<" "<<p->getNodeIn()->getId()<<" "<<p->getNodeOut()->getId()<<" "<<p->getRadius()*1e6<<" "<<p->getLength()*1e6<<" "<<p->getFlow()<<endl;
        }
    }
    ...
    {
        node* n=getNode(i);
        file2<<n->getId()<<" "<<n->getXCoordinate()*1e6<<" "<<n->getYCoordinate()*1e6<<" "<<n->getZCoordinate()*1e6<<endl;
    }
    ...
    while(timeSoFar<simulationTime)
    {
        ...
        for(int i=0;i<totalParticles;++i)
        {
            ...
            if(!p->getClosed())
            {
                ...
                {
                    double fraction=pp->getFlow()>0?p->getPorePosition():1-p->getPorePosition();
        ...
        for(int i=0;i<totalParticles;++i)
        {
            ...
            if(!p->getClosed())
            {
                ...
                if(abs(pp->getFlow())>1e-20)
                {
                    double newPosition=p->getPorePosition()+timeStep*pp->getFlow()/pp->getVolume();

                    double xOut=pp->getNodeOut()==0?pp->getNodeIn()->getXCoordinate()-pp->getLength():pp->getNodeOut()->getXCoordinate();
                    double xIn=pp->getNodeIn()==0?pp->getNodeOut()->getXCoordinate()+pp->getLength():pp->getNodeIn()->getXCoordinate();
                    double yOut=pp->getNodeOut()==0?pp->getNodeIn()->getYCoordinate():pp->getNodeOut()->getYCoordinate();
                    double yIn=pp->getNodeIn()==0?pp->getNodeOut()->getYCoordinate():pp->getNodeIn()->getYCoordinate();
                    double zOut=pp->getNodeOut()==0?pp->getNodeIn()->getZCoordinate():pp->getNodeOut()->getZCoordinate();
                    double zIn=pp->getNodeIn()==0?pp->getNodeOut()->getZCoordinate():pp->getNodeIn()->getZCoordinate();
                    ...
                    {
                        if(!pp->getVisited())
                            totalVisitedVolume+=pp->getVolume();
                        pp->setVisited(true);
                        ...
                        else
                        {
                            ...
                            for(unsigned j=0;j<connectedPores.size();++j)
                            {
                                ...
                                if(!np->getClosed())
                                {
                                    if((np->getNodeIn()==n && np->getFlow()<0) || (np->getNodeOut()==n && np->getFlow()>0))
                                    {
                                        cumulutativeProbability+=abs(np->getFlow())/n->getFlow();
                ...
                if(p->getPorePosition()==1 &&getPore(p->getPoreID()-1)->getFlow()>0 || p->getPorePosition()==0 && getPore(p->getPoreID()-1)->getFlow()<0 )cout<<"ff"<<endl;
            }
        }
        ...
        if(injectParticles && timeToInject>injectionInterval)
        {
            ...
            p->setPorePosition(0);
            p->setXCoordinate(pp->getNodeOut()==0?pp->getNodeIn()->getXCoordinate()-pp->getLength():pp->getNodeOut()->getXCoordinate());
            p->setYCoordinate(pp->getNodeOut()==0?pp->getNodeIn()->getYCoordinate():pp->getNodeOut()->getYCoordinate());
            p->setZCoordinate(pp->getNodeOut()==0?pp->getNodeIn()->getZCoordinate():pp->getNodeOut()->getZCoordinate());
            p->setId(totalParticles);
            tableOfParticles.push_back(p);
            timeToInject=0;

            file3<<totalParticles<<" "<<totalVisitedVolume/totalPoresVolume<<endl;
        }
        ...
        if(extractData && totalCurrentParticles>=1)
        {
            ...
            extractParticleFlowResults(endTime-startTime, timeSoFar, simulationTimeElapsed, outputCount, true);
        }
        ...
        {injectionInterval/=2.;timeToDoubleFlow+=originalStep;}
