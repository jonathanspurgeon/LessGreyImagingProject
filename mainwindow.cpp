...
void MainWindow::exportNetworkDataFromGUI()
{
    ...
    settings.setValue("extractionTimestep",ui->extractionTimeStepSpinBox->text());
    settings.endGroup();
...
void MainWindow::exportTwoPhaseDataFromGUI()
{
    ...
    settings.setValue("drugFlowWithoutDiffusion",ui->drugFlowWithout->isChecked());
    ...
    settings.setValue("angiogenesisTumour",ui->angiogenesisOnLattice->isChecked());
    settings.setValue("angiogenesisRetina",ui->angiogenesisOffLattice->isChecked());
    settings.setValue("cellProliferationSimulation",ui->cellProliferation->isChecked());
    settings.endGroup();
    ...
    settings.setValue("bolusDuration",ui->twoPhasePrecisionFactorLineEdit->text());
...
{
    ui->widget_3d->setNodeBodies(ui->node3DCheckBox->isChecked());
    ui->widget_3d->updateGL();
}
...
void MainWindow::on_renderNetworkButton_clicked()
{
    if(ui->loadNetworkStatusRadioButton->isChecked())
    {
        ...
        ui->widget_3d->setPhasePoresPath(ui->pathToPhasePoresLineEdit->text().toStdString());
        ui->widget_3d->setPhaseNodesPath(ui->pathToPhaseNodesLineEdit->text().toStdString());
        ui->widget_3d->setPhaseBlockPath(ui->pathToPhaseBlocksLineEdit->text().toStdString());
    }
...
void MainWindow::on_cells3DCheckBox_clicked(bool checked)
{
    ui->widget_3d->setCells(checked);
    ui->widget_3d->updateGL();
}
