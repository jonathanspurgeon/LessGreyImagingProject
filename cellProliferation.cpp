...
void network::runCellProliferation()
{
...
***************************************************************************** */
double network::aleatorio(const double a, const double b)
{
...
****************************************************************************** */
double network::box_muller(const double m, const double s)
{
  double w, y1;
  double x1, x2;
  static double y2;
...
****************************************************************************/
double network::DISTANCE(const Cell& c1, const Cell& c2)
{
  double dist =
 ...
 */
double network::DISTANCE(const Cell& cell, const Fibre& fibre)
{
...
/**
   ...
   -> otherwise, we take the projection of cell center onto the fibre
 */
vector<double> network::DISTANCE(const Cell& cell, pore *p)
{
  double px2=p->getNodeIn()->getXCoordinate()*1e6;
  double py2=p->getNodeIn()->getYCoordinate()*1e6;
  double pz2=p->getNodeIn()->getZCoordinate()*1e6;
  double px1=p->getNodeOut()->getXCoordinate()*1e6;
  double py1=p->getNodeOut()->getYCoordinate()*1e6;
  double pz1=p->getNodeOut()->getZCoordinate()*1e6;

  double dx = px2 - px1;
  double dy = py2 - py1;
  double dz = pz2 - pz1;

  double cellX=cell.position[0];
  double cellY=cell.position[1];
  double cellZ=cell.position[2];

  double vesselLength_sq=pow(p->getLength()*1e6,2);

  double pdx, pdy, pdz;	// vector pd from point 1 to cell point
  double dot, dsq, dsx, dsy, dsz;

  pdx = cellX - px1;		// vector from pt1 to cell point.
  pdy = cellY - py1;
  pdz = cellZ - pz1;
  ...
  if( dot < 0.0f)
  {
      // distance squared to the pore nodeOut:
      dsq = pow(cellX-px1,2)+pow(cellY-py1,2)+pow(cellZ-pz1,2);
      dsx = -pdx;
      dsy = -pdy;
      dsz = -pdz;
  }

  if( dot > vesselLength_sq)
  {
      // distance squared to the pore nodeIn:
      dsq = pow(cellX-px2,2)+pow(cellY-py2,2)+pow(cellZ-pz2,2);
      dsx = dx-pdx;
      dsy = dy-pdy;
      dsz = dz-pdz;
  }

  if( dot >= 0.0f && dot <= vesselLength_sq)
  {
      // distance squared to the cylinder axis:
      dsq = (pdx*pdx + pdy*pdy + pdz*pdz) - dot*dot/vesselLength_sq;
      dsx = dot*dx/vesselLength_sq-pdx;
      dsy = dot*dy/vesselLength_sq-pdy;
      dsz = dot*dz/vesselLength_sq-pdz;
  }
  vector<double> dist;
  dist.push_back(sqrt(dsq));
  dist.push_back(dsx);
  dist.push_back(dsy);
  dist.push_back(dsz);
...
void network::init(string f)
{
  ...
  //override with nework data
  params.boxesx=Nx;
  params.boxesy=Ny;
  params.boxesz=Nz;

  params.lattice_length_x=Nx*length*1e6;
  params.lattice_length_y=Ny*length*1e6;
  params.lattice_length_z=Nz*length*1e6;

  params.ic_cell_x.clear();
  params.ic_cell_y.clear();
  params.ic_cell_z.clear();
  params.ic_cell_x.push_back(params.lattice_length_x/2+5);
  params.ic_cell_y.push_back(params.lattice_length_y/2+5);
  params.ic_cell_z.push_back(params.lattice_length_z/2+5);

  if(Nz==1) params.dimension=2;
...
/*****************************************************************************/
void network::allocate_compare_box()
{
 ...
 *********************************** */
void network::setElementsInBox(const Mesh& _mesh)
{
  if (_mesh.dim==2) {
...
void network::checkElementsInBoxes()
{
...
void network::oxy_in_cell(Cell& cell) {
 ...
 ************************************************************************************/
void network::reverse_phenotype(Cell& cell) {
...
*************************************************************************************/
void network::cell_birth(Cell& cell)
{
  ...
  if(cell.contacts<16 && // birth allowed only if n.contact < threshold
    ...
    cout << " *** error in network::birthday_phenotype : "
...
/*****************************************************************************/
// void network::cell_fibres_interaction(Cell& cell) {
...
//    }
  ...
  //cout<<cell.contact_vessels.size()<<endl;
  for(unsigned int kk=0; kk<cell.contact_vessels.size(); kk++) {
    ...
    pore* vess=getPore(cell.contact_vessels[kk]-1);

    vector<double>dist=DISTANCE(cell,vess);
    double dist_cell_vessel = dist[0];
    double c_v_disp = cell.radius + vess->getRadius()*1e6 - dist_cell_vessel;
    ...
    //cout << dist_cell_vessel <<" "<<c_v_x<<" "<<c_v_y<<" "<<c_v_z<< endl;
    
    if(dist_cell_vessel>0.0001)
    {
        c_v_x = c_v_x/dist_cell_vessel;
        c_v_y = c_v_y/dist_cell_vessel;
        c_v_z = c_v_z/dist_cell_vessel;
...
*************************************************************************/
void network::movement(const Cell& cell,
...
// void network::fibre_induced_movement(Cell& cell)
// {
...
/**********************************************************************************************/
void network::compute_cell_fibres_contact(const int u,
...
/**********************************************************************************************/
//void network::compute_cell_vessels_contact(const int u,
//		 const int v,const int w)
//{
//  unsigned int total_cells_in_box = this->boxes_A[u][v][w].cells.size();
//  for(unsigned int i=0; i<total_cells_in_box; i++){
...
/**********************************************************************************************/
/* calculate the local distance between cells and vessels - Ahmed NEW 11/08/17 */
/**********************************************************************************************/
void network::compute_cell_vessels_contact(const int u,
         const int v,const int w)
{
    for(int ii=-1;ii<2;ii++)
      ...
      block* b=getBlock(u+ii,v+jj,w+kk);
      if(b!=0){

      auto& neigh=b->neighbooringVesselsArea();
      ...
      for(auto iterator : neigh)
      {
          ...
          pore* p=getPore(poreID-1);
          if(!p->getClosed())
          {
              // compute distance between cells and vessels
              double cell_vessel_min_dist = DISTANCE(this->boxes_A[u][v][w].cells[i],p)[0];
              double cell_rad = this->boxes_A[u][v][w].cells[i].radius;
              double vessel_rad = p->getRadius()*1e6;

              //cout<<cell_vessel_min_dist<<" "<<cell_rad<<" "<<vessel_rad<<endl;

              if(cell_vessel_min_dist < (cell_rad+vessel_rad)){
              if (params.verbose>1) {
                cout << " cell " << this->boxes_A[u][v][w].cells[i].name << " in contact with vessel "<< endl;
                cout << " distance between the cell and vessel is " << cell_vessel_min_dist-(cell_rad+vessel_rad) << endl;
              }
              this->boxes_A[u][v][w].cells[i].contact_vessels.push_back(p->getId());
              this->boxes_A[u][v][w].cells[i].interaction_phenotype = 1.0;
              }
          }
      }
...
/**********************************************************************************************/
void network::contact_forces(const int u,
         const int v,const int w,
...
/*****************************************************************************/
void network::compare_elements(int u, int v, int w,
...
int network::search_in_box(const vector<int>& box_number,
...
void network::count_cells_per_box()
{
      for(int k=this->minx; k<=this->maxx; k++) {
    for(int l=this->miny; l<=this->maxy; l++){
      for(int n=this->minz; n<=this->maxz; n++) {
...
void network::count_cells_per_type()
{
  ...
  if ( (this->totNorm[nt] + this->totHypo[nt] + this->totDead[nt]) < this->total_no_of_cells ) {
    cout << " ERROR in network::count_cells_per_type(): not all cells found ! " << endl;
    this->end();
    exit(1);
  }
...
void network::writeVtk(string filename,unsigned int onlyCoord)
{
...
void network::writeBoxesVtk(string filename)
{
  ofstream ofile;
...
// CICELY NEW
void network::writeFibresVtk(string filename)
{
  ofstream ofile;
  ofile.open(filename.c_str());
...
void network::fibre_degradation(Cell& cell, Fibre& fibre, double cell_fibre_min_dist) {
...
int network::get_sub_domain_id(double x,double y,double z)
{
  ...
  default:
    {
      cout << " ** ERROR in network::get_sub_domain_id: sub_domain_model_type = "
