...
void network::setupModel()
{
    ...
    if(networkSource==6) //cell network
    {
        Nz=Ny;
        //createParentVessel3D();
        setupRegularModel();
        init("Input Data/cell_data.txt");
    }

    //Build tissue for certain network types OR if requested
    if(networkSource==4 || networkSource==5 || networkSource==6 || buildTissue)
        generateTissue();
...
void network::runSimulation()
{
    ...
    if(drugFlowWithDiffusion && networkSource<6)
        runDrugFlowWithDiffusion();
    if(particleFlow && networkSource<6)
        runParticleFlow();
    if(angiogenesisTumour && networkSource==4)
        runAngiogenesisOnLattice();
    if(angiogenesisRetina && networkSource==5)
        runRetinaModel();
    if(cellProliferationSimulation && networkSource==6)
        runCellProliferation();
}
...
{
    if(i<0 || i>meshSizeX-1 || j<0 || j>meshSizeY-1 || k<0 || k>meshSizeZ-1)
        return 0;
...
int network::getTotalBlocks() const
{
    return totalBlocks;
}
...
int network::getTotalParticles() const
{
