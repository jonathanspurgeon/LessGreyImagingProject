...
// Data extraction
void network::extractDrugFlowResults(double timeElapsed, double timeSoFar, double &simulationTimeElapsed, int &outputCount, bool forceExtraction)
{
    if(timeElapsed>simulationTimeElapsed || forceExtraction)
    {
        ...
        string path="Results/Network_Status/conc_pores"+boost::lexical_cast<std::string>(1000000000+outputCount)+".txt";
        ...
        {
            string path="Results/Network_Status/conc_nodes"+boost::lexical_cast<std::string>(1000000000+outputCount)+".txt";
        ...
        path="Results/Network_Status/conc_blocks"+boost::lexical_cast<std::string>(1000000000+outputCount)+".txt";
...
void network::extractParticleFlowResults(double timeElapsed, double timeSoFar, double &simulationTimeElapsed, int &outputCount, bool forceExtraction)
{
    if(timeElapsed>simulationTimeElapsed || forceExtraction)
    {
//        ofstream ofs1;
...
//        string path="Results/Network_Status/particles"+boost::lexical_cast<std::string>(1000000000+outputCount)+".txt";
...
//            if(!p->getClosed()  )
//            {
//                file1<<p->getId()<<" "<<p->getXCoordinate()<<" "<<p->getYCoordinate()<<" "<<p->getZCoordinate()<<endl;
//            }
//        }
...
//        ofs1.close();
        ...
        for(int i=0;i<totalParticles;++i)
        {
            ...
            if(!p->getClosed()  )
            {
                ofs1<<outputCount<<"\t"<<timeSoFar<<"\t"<<p->getId()<<"\t"<<p->getXCoordinate()*1e6<<"\t"<<p->getYCoordinate()*1e6<<"\t"<<p->getZCoordinate()*1e6<<endl;
            }
        }
