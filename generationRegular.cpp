    ...
    for(int i=0;i<totalNodes;++i)
    {
        ...
        for(int i=0;i<totalPores;++i)
        {
            ...
            if(nodeIn==n || nodeOut==n)
            {
                ...
                if(nodeIn==n) neighboorsNodes.push_back(nodeOut==0?-1:nodeOut->getId());
                if(nodeOut==n) neighboorsNodes.push_back(nodeIn==0?-1:nodeIn->getId());
                connectionNumber++;
            }
        }
    ...
    for (int i = 0; i < totalPores; ++i)
    {
        ...
        if(!p->getClosed())
        {
            ...
            if(radiusDistribution==4)
                p->setRadius(weibull(minRadius,maxRadius,normalMuParameter,normalSigmaParameter));
...
void network::assignVolumes()
{
    ...
    for (int i = 0; i < totalNodes; ++i)
    {
        ...
        if(!n->getClosed())
        {
            double volume=poreVolumeConstant*n->getLength()*pow(n->getRadius(),poreVolumeExponent)/(4*n->getShapeFactor())*pow(10,(6*poreVolumeExponent-12));
            n->setVolume(volume);
...
void network::generateTissue()
{
    ...
    if(networkSource==1 || networkSource==4 || networkSource==5 || networkSource==6)
    {
        meshSizeX=Nx;
        meshSizeY=Ny;
        meshSizeZ=Nz;
    ...
    for(int i=0;i<totalBlocks;++i)
    {
        ...
        if(!n->getInlet())neighboors.push_back(getBlock(n->getX()-1,n->getY(),n->getZ()));
        if(!n->getOutlet())neighboors.push_back(getBlock(n->getX()+1,n->getY(),n->getZ()));
        if(!n->getInletY())neighboors.push_back(getBlock(n->getX(),n->getY()-1,n->getZ()));
        if(!n->getOutletY())neighboors.push_back(getBlock(n->getX(),n->getY()+1,n->getZ()));
        if(!n->getInletZ())neighboors.push_back(getBlock(n->getX(),n->getY(),n->getZ()-1));
        if(!n->getOutletZ())neighboors.push_back(getBlock(n->getX(),n->getY(),n->getZ()+1));
    ...
    if(networkSource==3)
        tissueNetworkCollisionAnalysis();
}
...
void network::tissueNetworkCollisionAnalysis()
{
           ...
           for(int x=-abs(ndx);x<abs(ndx);x++)
               for(int y=-abs(ndy);y<abs(ndy);y++)
                   for(int z=-abs(ndz);z<abs(ndz);z++)
                   {
                       ...
                       if (b!=0 && neighbooringBlocks.find(b->getId()) == neighbooringBlocks.end())
                       {
                           ...
                           for(int xx=0;xx<2;xx++)
                               for(int yy=0;yy<2;yy++)
                                   ...
                                   {
                                       double testX=b->getXCoordinate()+(xx==0?1:-1)*b->getHx()/2.;
                                       double testY=b->getYCoordinate()+(yy==0?1:-1)*b->getHy()/2.;
                                       double testZ=b->getZCoordinate()+(zz==0?1:-1)*b->getHz()/2.;
                                       ...
                                       if( dot > 0.0f && dot < p->getLength_sq() )
                                       {
                                           ...
                                           dsq = (pdx*pdx + pdy*pdy + pdz*pdz) - dot*dot/p->getLength_sq();

                                           if( dsq < p->getRadius_sq() )
                                               intersection_number++;
                                       }
                                   }
                           ...
                           {
                               double area=double(intersection_number)/9.*tools::pi()*coeff/4.;
                               b->neighbooringVesselsArea()[p->getId()]=area;
                               p->neighbooringBlocksArea()[b->getId()]=area;
                           }

                           if(intersection_number==9)
                               b->setClosed(true);
   ...
   for (int j = 0; j < totalPores; ++j)
   {
       ...
       if(!p->getClosed())
       {
           ...
           if(neigh.size()==0)
           {
               ...
               double blocksNumber=(abs(cx1-cx2)+1)*(abs(cy1-cy2)+1)*(abs(cz1-cz2)+1);
   ...
   double adjustingFactor=(voxelVolume-volumeOfDisconnectedBlocks-totalVesselVolume)/volumeOfConnectedBlocks;
...
void network::tissueNetworkCollisionAnalysisRegular()
{
    for(int i=0;i<totalPores;++i)
    {
        ...
        if(!p->getClosed())
        {
            ...
            if(nodeIn!=0)
            {
                block* b1=getBlock(nodeIn->getIndexX(),nodeIn->getIndexY(),nodeIn->getIndexZ());
                b1->neighbooringVesselsArea()[p->getId()]=tools::pi()*p->getRadius()*p->getLength();
                b1->setVolume(b1->getVolume()-p->getVolume()/2);
                b1->setConnectedToVessel(true);
                p->neighbooringBlocksArea()[b1->getId()]=tools::pi()*p->getRadius()*p->getLength();
            }
            if(nodeOut!=0)
            {
                block* b2=getBlock(nodeOut->getIndexX(),nodeOut->getIndexY(),nodeOut->getIndexZ());
                b2->neighbooringVesselsArea()[p->getId()]=tools::pi()*p->getRadius()*p->getLength();
                b2->setVolume(b2->getVolume()-p->getVolume()/2);
                b2->setConnectedToVessel(true);
                p->neighbooringBlocksArea()[b2->getId()]=tools::pi()*p->getRadius()*p->getLength();
            }
        }
    }
    ...
    if(tissueCircular)
    {
        for (int i=0;i<totalPores;++i)
        {
            ...
            if(!p->getClosed())
            {
                ...
                if(pow(xc-x,2)+pow(yc-y,2)+pow(zc-z,2)<pow(tissueCircularR,2))
                    p->setMembranePermeability(PVT*100);
            }
        }
